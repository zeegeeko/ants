...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while current_place.entrance != hive and len(bees) == 0:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        #iterate to place of min range
        for i in range(1,self.min_range):
            current_place = current_place.entrance

        #check for nearest bee past min range
        while current_place.entrance != hive and len(bees) == 0:
            current_place = current_place.entrance
            bees = current_place.bees
        return random_or_none(bees)
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        #check for nearest bee up to max range
        while current_place.entrance != hive and len(bees) == 0 and count < self.max_range:
            current_place = current_place.entrance
            bees = current_place.bees
            count += 1
        return random_or_none(bees)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting == 0:
            if len(bee_place) != 0:
                self.eat_bee(random_or_none(bee_place))
    ...
    def action(self, colony):
        ...
        while current_place != None:
            if current_place.ant:
                if isinstance(current_place.ant, BodyguardAnt) and current_place.ant.ant:
                    add_buff(current_place.ant.ant)
                else:
                    add_buff(current_place.ant)
            #move to next place
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** REPLACE THIS LINE ***"
    current_effect = effect(Bee.action) #apply the effect on action
    bee.effect[current_effect] = duration #track status effects applied on bee instance
    default_action = bee.action #save default bee action

    def effect_action(colony):
        nonlocal current_effect
        current_effect(bee, colony)
        effect_list = list(bee.effect.keys())
        remain_dict = bee.effect.copy()

        #decrement the effect counter each call
        for i in bee.effect:
            if bee.effect[i] >= 1:
                bee.effect[i] -= 1
            else:
                remain_dict.pop(i, None)
                effect_list = list(remain_dict.keys())

        #if all effects have expired, restore the default bee action
        if len([i for i in list(bee.effect.values()) if i > 0]) == 0:
            bee.action = default_action
        elif bee.effect[current_effect] == 0 and len(effect_list) > 1: #If one effect has expired change effect
            current_effect = effect_list[0]
